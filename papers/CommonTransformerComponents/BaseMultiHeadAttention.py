...
class BaseMultiHeadAttention(nn.Module, ABC):
    ...
    def forward(self, Q, K, V, mask=None, return_attention=False):
        ...
        key = key.view(batch_size, self.n_heads // self.group_sizes, self.group_sizes, seq_length, self.dk).mean(dim=2)
        value = value.view(batch_size, self.n_heads // self.group_sizes, self.group_sizes, seq_length, self.dv).mean(dim=2)
